...
export function InstitutionEditForm({ institution }: InstitutionEditFormProps) {
  ...
  // Geo data
  const { data: regions } = useSWR<GeoDTO[]>("/api/geo/regions", fetcher);
  ...
  const { data: prefectures } = useSWR<GeoDTO[]>(
    selectedRegion ? `/api/geo/regions/${selectedRegion}/prefectures` : null,
    fetcher
  );
  const { data: communes } = useSWR<GeoDTO[]>(
    selectedPrefecture ? `/api/geo/prefectures/${selectedPrefecture}/communes` : null,
    fetcher
  );
  ...
  const onSubmit = async (data: InstitutionRequest) => {
    ...
    try {
      ...
      const response = await authMutate(`/api/institutions/${institution.id}`, {
        method: "PUT",
